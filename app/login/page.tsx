...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-950 text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md rounded-2xl bg-slate-800/90 border border-slate-700 p-6">
        <h1 className="text-2xl font-bold">Login</h1>
        <p className="text-slate-300 mt-1">Access your smart resume dashboard.</p>
        ...
        <form onSubmit={handleLogin} className="mt-5 space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full rounded-lg bg-cyan-500 px-3 py-2 font-semibold text-slate-900 hover:bg-cyan-400 disabled:opacity-60">
            {loading ? "Logging in..." : "Login"}
          </button>
        </form>

        <p className="mt-4 text-slate-300 text-sm">
          New here? <Link href="/register" className="text-cyan-300 hover:text-cyan-200">Create account</Link>
        </p>
      </div>
    </div>
  );
